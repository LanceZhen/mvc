...
class Smarty_Internal_Utility
{
    ...
    public static function testInstall(Smarty $smarty, &$errors=null)
    {
        ...
        if (!$_core_plugins_available) {
            $status = false;
            $message = "WARNING: Smarty's own Lib/plugins is not available";
            if ($errors === null) {
                echo $message . ".\n";
        ...
        if (is_dir($source)) {
            ...
            if ($expected) {
                $status = false;
                $message = "FAILED: files missing from Lib/sysplugins: ". join(', ', array_keys($expected));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
        ...
        if (is_dir($source)) {
            ...
            if ($expected) {
                $status = false;
                $message = "FAILED: files missing from Lib/plugins: ". join(', ', array_keys($expected));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
